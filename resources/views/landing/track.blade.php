            ...
            <div class="col-lg-8">
                ...
                <div class="card border-0 shadow-sm mb-4" style="border-radius: 12px; overflow: hidden;">
                    ...
                    <div class="card-body p-4">
                        ...
                        {{-- ── Payment Verification Notice (after payment submitted) ── --}}
                        @php
                            $payment = $application->payment;
                        @endphp
                        @if($application->latestStatus?->status?->name === 'Payment Verification')
                        @if($payment && $payment->proof_of_payment_status === 'rejected')
                        <div class="rounded-3 border mb-4 overflow-hidden" style="border-color:#dc3545 !important; border-width: 1px !important;">
                            <div class="px-4 py-2 fw-bold d-flex align-items-center gap-2"
                                style="background:#dc3545; color:#fff; font-size:.9rem;">
                                <i class="bi bi-x-circle-fill"></i> Payment Rejected — Revisions Required
                            </div>
                            <div class="px-4 py-3" style="background:#fff8f8;">
                                <p class="mb-0 text-danger fw-bold" style="font-size:.92rem;">
                                    <i class="bi bi-exclamation-triangle-fill me-2"></i>
                                    Your proof of payment was rejected. Please review the remarks below and upload the corrected file.
                                </p>
                            </div>
                        </div>
                        @else
                        <div class="rounded-3 border mb-4 overflow-hidden" style="border-color:#22c55e !important; border-width: 1px !important;">
                            <div class="px-4 py-2 fw-bold d-flex align-items-center gap-2"
                                style="background:#16a34a; color:#fff; font-size:.9rem;">
                                <i class="bi bi-check-circle-fill"></i> Payment Received — Under Verification
                            </div>
                            <div class="px-4 py-3" style="background:#f0fdf4;">
                                <p class="mb-0 text-success fw-semibold" style="font-size:.92rem;">
                                    <i class="bi bi-hourglass-split me-2"></i>
                                    Your proof of payment has been submitted and is currently being reviewed by our verifiers. We will notify you once it has been processed.
                                </p>
                            </div>
                        </div>
                        @endif
                        ...
                        @if($application->latestStatus?->status?->name === 'Awaiting Payment' || ($application->latestStatus?->status?->name === 'Payment Verification' && $payment && $payment->proof_of_payment_status === 'rejected'))
                        <div class="rounded-3 border mb-4 overflow-hidden" style="border-color:#fbbf24 !important; border-width: 1px !important;">
                            ...
                            <div class="px-4 py-3" style="background:#fffbeb;">
                                <div class="alert alert-warning border border-warning-subtle d-flex align-items-start gap-3 p-3 mb-3 bg-white" style="border-radius: 10px; color: #856404;">
                                    <i class="bi bi-info-circle-fill fs-5 mt-1 text-warning"></i>
                                    <div class="w-100">
                                        <h6 class="fw-bold mb-2" style="color: #856404; font-size: 1rem;">Payment Instructions & Steps</h6>
                        ...
                        @endif
                        @php
                        $isForUpdate = optional($application->latestStatus?->status)->name === 'For Update';

                        // Pre-compute rejected items for resubmission form
                        ...
                        // The CV carries its own status, separate from the service agreement.
                        $rejectedCvs = $application->user
                            ? $application->user->instructors->filter(fn($i) => $i->cv_path && in_array($i->cv_status, ['rejected','returned']))
                            : collect();
                        ...
                        <div class="mt-2 mb-4 p-4 border rounded-3" style="background:#fff8f8; border-color:#f5c6cb !important;">
                        ...
                        @else
                        <div class="mt-2 mb-4 p-4 border rounded-3 text-center" style="background:#fff8f8; border-color:#f5c6cb !important;">
                                    ...
                                    <div class="list-group-item px-3 py-3 d-flex flex-column flex-md-row align-items-start align-items-md-center justify-content-between gap-2">
                                        <div class="flex-grow-1">
                                            ...
                                            <div class="d-flex align-items-center gap-2 mb-1">
                                                ...
                                                <span class="badge" style="background:#fee2e2; color:#991b1b;"><i class="bi bi-x-circle me-1"></i>{{ $isForUpdate ? 'Requires Resubmission' : 'Rejected' }}</span>
                                                @else
                                            ...
                                            {{-- Evaluator remarks (only shown after notification is sent) --}}
                                            @if($isForUpdate && in_array($doc->status, ['returned','rejected']) && $doc->remarks)
                                        ...
                                        @if($isForUpdate && in_array($doc->status, ['returned','rejected']) && $doc->documentField?->input_type === 'file')
                                        <div class="mt-1">
                            ...
                            <div class="list-group list-group-flush">
                                ...
                                <div class="list-group-item px-3 py-3 d-flex flex-column flex-md-row align-items-start align-items-md-center justify-content-between gap-2" style="background-color: #fafbfe;">
                                    <div class="flex-grow-1">
                                        ...
                                        @if($isForUpdate && in_array($instructor->status, ['returned','rejected']) && $instructor->remarks)
                                        <div class="alert py-2 px-3 border-0 rounded mt-2 mb-0" style="background:#fee2e2; color:#7f1d1d; font-size:.85rem;">
                                            <strong><i class="bi bi-chat-left-text-fill me-1"></i>Evaluator Remarks:</strong>
                                    ...
                                    @if($isForUpdate && in_array($instructor->status, ['returned','rejected']))
                                    <div class="mt-1">
                                        <span class="badge" style="background:#fff3cd; color:#856404; font-size:.75rem;">
                                ...
                                <div class="list-group-item px-3 py-3 d-flex flex-column flex-md-row align-items-start align-items-md-center justify-content-between gap-2 ps-4">
                                    <div class="flex-grow-1">
                                        ...
                                        @if($isForUpdate && in_array($cred->status, ['returned','rejected']) && $cred->remarks)
                                        <div class="alert py-2 px-3 border-0 rounded mt-2 mb-0" style="background:#fee2e2; color:#7f1d1d; font-size:.85rem;">
                                            <strong><i class="bi bi-chat-left-text-fill me-1"></i>Evaluator Remarks:</strong>
                                    ...
                                    @if($isForUpdate && in_array($cred->status, ['returned','rejected']))
                                    <div class="mt-1">
                                        <span class="badge" style="background:#fff3cd; color:#856404; font-size:.75rem;">
                                            <i class="bi bi-arrow-down-circle me-1"></i>Attach replacement below
                                        </span>
